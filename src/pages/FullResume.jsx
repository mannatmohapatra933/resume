                  ...
                  {(data.socials || []).map((social, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3 text-on-surface-variant font-medium">
                        ...
                        <span className="truncate">{social.url.replace('https://', '').replace('http://', '')}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className={compact ? 'space-y-2' : 'space-y-3'}>
                  {(data.certificates || []).map((cert, i) => (
                    <div key={i} className="group">
                      {cert.url && cert.url !== "#" ? (
                        <a href={cert.url} target="_blank" className="hover:text-primary transition-colors">
                          <p className="text-[11px] font-bold text-on-surface leading-tight flex items-center gap-1">
                            {cert.title}
                ...
                {visibility.certificates && (
                  <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1">
                      {(data.certificates || []).map((cert, i) => (
                        <div key={i} className="flex justify-between items-center border-l border-primary/10 pl-2 group">
                          <div>
                            {cert.url && cert.url !== "#" ? (
